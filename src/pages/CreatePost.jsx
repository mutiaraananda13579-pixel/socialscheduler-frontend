...
export default function CreatePost() {
  ...
  const bgPreview = darkMode 
    ? "bg-slate-800/30 border-slate-700/30" 
    : "bg-gradient-to-r from-blue-50/80 to-purple-50/80 border-blue-100/50";
  ...
  return (
    <DashboardLayout>
      <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <p className={`${textSecondary} mt-2`}>
                Buat konten baru untuk dibagikan ke audiens Anda
              </p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {success && (
              <div className={`flex items-start gap-3 p-4 ${
                ...
                <span>Post berhasil dibuat! Mengalihkan...</span>
              </div>
            )}
            ...
            <div className={`flex flex-col sm:flex-row gap-3 pt-4 border-t ${borderColor}`}>
              ...
              >
                {loading ? (
                  ...
                  status === "draft" ? "Simpan sebagai Draft" : "Jadwalkan Post"
                )}
              </button>
        ...
        {/* Preview Card - LEBIH BAGUS DI LIGHT MODE */}
        <div className={`mt-6 p-5 ${bgPreview} border rounded-xl transition-all duration-300 ${
          darkMode ? "" : "shadow-md hover:shadow-lg"
        }`}>
          <div className="flex items-center gap-2 mb-3">
            <span className={`text-xs font-semibold uppercase tracking-wider ${textMuted}`}>Preview</span>
            <div className={`flex-1 h-px ${darkMode ? "bg-slate-700/50" : "bg-gray-200"}`}></div>
          </div>
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 shrink-0 flex items-center justify-center text-white font-bold shadow-lg">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-2">
                <div className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusColor(status)}`}>
                  {getStatusIcon(status)}
